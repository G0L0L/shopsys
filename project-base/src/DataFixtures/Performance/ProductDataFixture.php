...
use App\DataFixtures\Demo\ProductDataFixtureCsvReader;
use App\DataFixtures\Demo\ProductDataFixtureLoader;
...
class ProductDataFixture
{
     ...
     * @param \App\DataFixtures\ProductDataFixtureReferenceInjector $productDataReferenceInjector
    ...
    private function setArrayPointerByKey(array &$array, $key)
    {
        ...
        while (key($array) !== $key) {
            if (empty(each($array)) === true) {
                throw new \App\DataFixtures\Performance\Exception\UndefinedArrayKeyException($key);
            }
        }
    }
}
